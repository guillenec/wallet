      ...
      items.map((element) => {
        ...
        return (
          <div key={element.category + uuidv4()}>
            ...
            >
              <span className={`${selectedItem && selectedItem.name === element.name ? 'bg-hoverBotonSubmenu text-colorFuente2Submenu' : `${element.color}`} w-[30px] h-[30px] flex justify-center items-center rounded-full text-colorFuente2Submenu text-[1.2rem] text-white transition-all duration-300 ease-in`}><ion-icon name={element.icon} /></span>
              {element.name}
            ...
            <div className='w-full h-[2px] bg-bgSubmenu rounded-sm opacity-40' />
          </div>
        )
      })
    }
